...
# NOVA ADIÇÃO: Calcular proporção mensal em relação ao total
faturamento_total = df_mensal['Custo'].sum()
df_mensal['Proporcao_Percentual'] = (df_mensal['Custo'] / faturamento_total) * 100
# Aplicar percentuais ao valor de R$ 80.500
valor_base = 80500
df_mensal['Valores_Linha_Vermelha'] = (df_mensal['Proporcao_Percentual'] / 100) * valor_base
...
# NOVA ADIÇÃO: Linha vermelha com valores baseados em R$ 80.500
ax1.plot(range(len(df_mensal)), df_mensal['Valores_Linha_Vermelha'], marker='s', color='red', linewidth=1, markersize=4)

# Rótulos acumulados
max_valor = df_mensal['Acumulado'].max()
offset = max_valor * 0.02  
for i, y in enumerate(df_mensal['Acumulado']):
    ax2.text(i, y + offset, f'R$ {int(y):,}'.replace(',', '.'), fontsize=8, rotation=90, va='bottom', ha='center', color='#fca903')

# NOVA ADIÇÃO: Rótulos da linha vermelha
for i, (percentual, valor) in enumerate(zip(df_mensal['Proporcao_Percentual'], df_mensal['Valores_Linha_Vermelha'])):
    ax1.text(i, valor + (ax1.get_ylim()[1] * 0.02), f'R$ {int(valor):,}'.replace(',', '.'), 
             fontsize=7, rotation=0, va='bottom', ha='center', color='red')
...
# NOVA ADIÇÃO: Legenda para identificar as linhas
from matplotlib.lines import Line2D
from matplotlib.patches import Rectangle
legend_elements = [
    Rectangle((0, 0), 1, 1, facecolor='lightgray', edgecolor='black', label='Desembolso Mensal'),
    Line2D([0], [0], color='#fca903', marker='o', label='Desembolso Acumulado'),
    Line2D([0], [0], color='red', marker='s', label='Desembolso Coordenação')
]
