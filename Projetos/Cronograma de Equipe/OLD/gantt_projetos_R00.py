...
ARQ_CORES_CLIENTES = 'cores_clientes.json'
...
def plotar_gantt_projetos(df_projetos, cores_dict, titulo, arquivo_saida):
    ...
    for _, row in df_projetos.iterrows():
        ...
        # Barra do projeto
        plt.barh(y=y_pos, width=duracao, left=inicio, height=0.6, 
...
if __name__ == "__main__":
    ...
    print(f'Total de tarefas ativas: {len(df)}')
    
    # Extrair estrutura de clientes e projetos
    print('\nExtraindo clientes e projetos...')
    ...
    print(f'Clientes encontrados: {len(clientes_projetos)}')
    for cliente, projetos in clientes_projetos.items():
        print(f'  - {cliente}: {len(projetos)} projeto(s)')
    
    # Organizar projetos em linhas
    print('\nOrganizando projetos no gráfico...')
    ...
    print(f'Total de projetos a plotar: {len(df_projetos)}')
